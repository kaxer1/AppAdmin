...
export default function FormFulltime2_7(props) {
    const { baseDatos: arrSelectListBDD, setOptionsQueryFT2_7, optionsQueryFT2_7 } = props
    ...
    const onSubmit = (e) => {
        ...
        if (formOk) {
            ...
            window.api.send("Api/CrearConexionClienteFulltime27", { data: formData });
            window.api.receive("CrearConexionClienteFulltime27", (data) => {
                console.log(data);
                if (!data.err) {
                    setOptionsQueryFT2_7( data )
                    setFormData(defaultValueForm())
                    setBtnDisable(false)
                    setSelectList("")
                    toast.success('Conexión creada existosamente');
    ...
    const renderSuccessConnection = () => {
        const { database, host, port, user } = optionsQueryFT2_7;
        return (
            <>
                ...
                <Button 
                    ...
                    variant="contained" 
                    color="primary"  
                    onClick={() => { setSuccessConnection(false); setOptionsQueryFT2_7({}) }}
                    endIcon={<BlockIcon />}>
                    Desconectar
                </Button>
            </>
